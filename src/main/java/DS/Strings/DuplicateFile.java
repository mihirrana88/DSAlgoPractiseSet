...
public class DuplicateFile {
    ...
    public void processString(String path, Map<String, List<String>> map){
        String[] pathContent = path.split(" ");
        String pathRoute = pathContent[0];
        for(int i=1; i<pathContent.length; i++){
            String content = pathContent[i].substring(pathContent[i].indexOf("(")+1, pathContent[i].indexOf(")"));
            String fileName = pathContent[i].substring(0, pathContent[i].indexOf("("));
            List<String> paths = null;
            if(map.containsKey(content)){
                paths = map.get(content);

            }else{
                paths = new ArrayList<>();
            }
            paths.add(pathRoute+"/"+fileName);
            map.put(content, paths);
        }
    }
}
