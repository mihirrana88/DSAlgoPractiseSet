...
public class SumRootToLeafNumbers {
    ...
    public int sumNumbers() {
        return sumRecursive(binaryTreeInput.root, 0);
    }

    public static int sumRecursive(TreeNode node, int sum) {
        if (node == null) {
            return 0;
        }
        ...
        return sumRecursive(node.left, sum) + sumRecursive(node.right, sum);
    }
